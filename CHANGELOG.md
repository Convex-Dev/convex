...
- Convex UCAN authority tokens now support the explicit `exp: null` form for non-expiring authority. JWT-encoded UCANs carry and require the supported `ucv` payload version and UCAN claim shape; generic JWT validation permissively treats `exp: null` as an omitted optional expiry. Proof links use UCAN 1.0 execution-time validity rather than requiring child validity windows to fit within their proofs.
