...
- Etch: version-3 stores can now be created and cleanly reopened with plaintext or AES-256-CTR data, optional encrypted indexes, per-file random salts and header-MAC key verification. Cipher policy and opaque caller secret material are supplied through immutable `EtchConfig` values; dirty v3 files continue to fail fast pending explicit recovery or repair.
- Etch: immutable compiled `EtchConfig` values can now be constructed from strict JSON-style maps or typed parameters and supplied when creating or opening an Etch instance, while existing creation methods retain inferred, runtime-appropriate defaults. Public file-format values are available from `EtchConstants` for external readers and tooling.
