...
## [0.8.8-SNAPSHOT] - Unreleased

### Added

- CVM: new `cat` core function — raw byte concatenation of BlobLike values (Blobs, Strings, Addresses, Hashes, Keywords, Symbols). The result type follows the first non-nil argument (a String, Keyword or Symbol yields a String; any other BlobLike yields a Blob), and it never casts: each argument contributes its raw bytes — a String its UTF-8, a Keyword/Symbol its name — so `(cat 0x "cafe")` appends UTF-8 bytes where `(blob "cafe")` hex-parses. `nil` arguments are skipped. Pairs with `slice` for packed-blob state patterns. Installed by the protocol version 1 upgrade (#633).
- CVM: new `splice` core function — `(splice dst offset src)` overwrites the bytes of `dst` (a Blob or String) at byte `offset` with the raw bytes of `src`, returning a value of `dst`'s type. The write may extend past the end (growing the result; writing at `(count dst)` appends); `offset` outside `[0, (count dst)]` is a `:BOUNDS` error. `nil` is treated as empty (a `nil` `src` is a no-op). Cost is proportional to `(count src)` plus one chunk, sharing the untouched structure of large Blobs, so small edits to big packed-blob state are cheap. Installed by the protocol version 1 upgrade (#632).
