...
- Consensus: a peer no longer confirms a block dated beyond its own clock plus a small skew allowance (`MAX_BLOCK_FORWARD`, 30s), so a future-dated block cannot teleport the consensus clock forward — closing a clock-manipulation weakness amplified by the upgrade mechanism (firing scheduled upgrades early). Peer-local confirmation policy in belief merge, not a validity rule, so replay determinism is unchanged (#595, see `convex-core/docs/CONSENSUS.md`).
