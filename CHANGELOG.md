...
- Etch: immutable compiled `EtchConfig` values can now be constructed from strict JSON-style maps or typed parameters and supplied when creating or opening an Etch instance, while existing creation methods retain inferred, runtime-appropriate defaults.
