...
- Etch: immutable compiled `EtchConfig` values can now be constructed from strict JSON-style maps or typed parameters and supplied when creating or opening Etch instances and stores, including peer and CLI caller paths, while existing creation methods retain inferred, runtime-appropriate defaults. Public file-format values are available from `EtchConstants` for external readers and tooling.
