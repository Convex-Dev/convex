 ...
 *
 * Counter value is stored as Index&lt;AString, AVector&lt;ACell&gt;&gt; where each replica maps
 * to [positive, negative] counts. The displayed value is sum(positive) - sum(negative).
 *
 * Merge takes max per replica per column (PN-Counter CRDT).
 */
public class KVCounter {
	...
	/**
	 * Merges two counter values. Takes max per replica per column.
	 */
	@SuppressWarnings("unchecked")
	public static ACell merge(ACell a, ACell b) {
		...
		MergeFunction<AVector<ACell>> replicaMerge = (ea, eb) -> {
		...
		return ia.mergeDifferences(ib, replicaMerge);
	}
}
