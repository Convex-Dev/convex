...
public class ParboiledReader extends BaseParser<ACell> {
	...
	protected Action<ACell> ListAddAction(Var<ArrayList<ACell>> expVar) {
	...
	@SuppressWarnings("unchecked")
	public <T extends ACell> ASequence<T> popNodeList() {
		ACell o = pop();
	...
	@SuppressWarnings("unchecked")
	public <T extends ACell> T popValue() {
		ACell o = pop();
		if (o instanceof Syntax) o = ((Syntax) o).getValue();
		return (T) o;
	}
	 ...
	 *                   Syntax Objects
	 */
	public ParboiledReader(Boolean wrapSyntax) {
		this.wrapSyntax = wrapSyntax;
	}

	// Use a ThreadLocal reader because instances are not thread safe
	private static final ThreadLocal<ParboiledReader> formReader = new ThreadLocal<ParboiledReader>() {
		@Override
		protected ParboiledReader initialValue() {
			return Parboiled.createParser(ParboiledReader.class, false);
		}
	};

	// Use a ThreadLocal reader because instances are not thread safe
	private static final ThreadLocal<ParboiledReader> syntaxReader = new ThreadLocal<ParboiledReader>() {
		@Override
		protected ParboiledReader initialValue() {
			return Parboiled.createParser(ParboiledReader.class, true);
		}
	};
...
//				sb.append(result.parseTreeRoot);
	...
	public static <R extends ACell> R read(String source) {
		ParboiledReader reader = formReader.get();
	...
	public static Syntax readSyntax(String source) {
		ParboiledReader reader = syntaxReader.get();
		reader.tempSource = source;
