...
public final class LogWatchAPI extends ABaseAPI {
	...
	private record Subscription(SseConnection connection, Filter filter, Format format,
	...
	private static Format parseFormat(Context ctx) {
		List<String> values=ctx.queryParams("format");
		if (values.isEmpty()) return Format.JSON;
	...
	private void subscribe(SseConnection connection, Filter filter, Format format) {
	...
	private void distribute(long sourceGeneration, LogEvent event) {
		...
		for (Subscription subscription:subscriptions.values()) {
			...
			try {
				String data;
				if (subscription.format()==Format.JSON) {
					if (json==null) json=LogWatch.encode(event,Format.JSON);
					data=json;
				} else {
					if (cvx==null) cvx=LogWatch.encode(event,Format.CVX);
					data=cvx;
				}
