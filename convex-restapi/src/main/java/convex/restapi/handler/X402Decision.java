 ...
 * @param requirements Payment requirements when kind is CHARGE, else null
 ...
 */
public record X402Decision(Kind kind, PaymentRequirements requirements, String description,
		int status, String message) {

	public enum Kind {
		/** Serve the request without payment (e.g. credit was consumed) */
		ALLOW,
		/** Require an x402 payment satisfying the given requirements */
		CHARGE,
	...
	public static X402Decision charge(PaymentRequirements requirements, String description) {
		...
		return new X402Decision(Kind.CHARGE, requirements, description, 402, null);
	}
